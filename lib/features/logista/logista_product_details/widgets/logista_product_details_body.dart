...
import 'package:comparador_de_precos/features/logista/logista_product_details/dialogs/atualizar_promotion_price_dialog.dart';
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AtualizarPromotionPrecoCubit(product, getIt()),
      child: Builder(
        builder: (context) {
          return Padding(
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          if ((product.preco.precoPromocional ?? 0) > 0) ...[
                            const GutterTiny(),
                            Text(
                              'Preco promocional: '
                              // ignore: lines_longer_than_80_chars
                              '${numberFormat.format(product.preco.precoPromocional)}',
                              style: const TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w600,
                              ),
                  ...
                  child: Column(
                    children: [
                      ...
                      TextButton.icon(
                        ...
                        onPressed: () async {
                          final result =
                              await ConfirmarAcaoDialog.show(context);
                      ...
                      TextButton.icon(
                        ...
                        label: const Text('Adicionar preço promocional'),
                        onPressed: () async {
                          final result =
                              await AtualizarPromotionPriceDialog.show(
                            context,
                            product,
                          );
                      ...
                      TextButton.icon(
                        ...
                        label: const Text('Remover preço promocional'),
                        onPressed: (product.preco.precoPromocional ?? 0) > 0
                            ? () async {
                                await context
