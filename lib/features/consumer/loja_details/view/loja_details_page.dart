...
class LojaDetailsPage extends StatefulWidget {
  const LojaDetailsPage({super.key, required this.loja});
...
class _LojaDetailsPageState extends State<LojaDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => LojaDetailsCubit(context.read<LojaRepository>()) // Assuming LojaRepository is provided via BlocProvider/RepositoryProvider higher up
        ..fetchLojaDetails(widget.loja.id), // Fetch details when page loads
      child: Scaffold(
        appBar: AppBar(
        ...
        body: BlocBuilder<LojaDetailsCubit, LojaDetailsState>(
          builder: (context, state) {
            if (state is LojaDetailsLoading && state is! LojaDetailsSuccess) { // Show loading only if no data yet
            ...
            return LojaDetailsBody(loja: widget.loja);
          },
        ),
      ),
    );
